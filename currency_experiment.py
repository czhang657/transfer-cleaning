...
import pandas as pd
import currency_experiment_functions as sef
import os
import random

# Get a list of symbols already run
already_run_path = 'currency/results/'  # Replace with the actual path to your folder

# Average all of the dataframes in temp_results folder
stock_path = "currency/currency_data/"  # Replace with the actual path to your folder
part_path = "currency/currency_data_partitions/"
...
def stock_processing(partitions, name, target_column="Close", task_type="regression", metric="mean_squared_error"):
    ...
    for index, partition in enumerate(partitions):
        ...
        output_path = f"currency/results/{name}_partition_{index}.csv"
