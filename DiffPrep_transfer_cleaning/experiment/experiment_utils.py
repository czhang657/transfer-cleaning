...
def save_result(result, model_dict, logger, params, save_dir, save_model=False):
    ...
    # Add this to experiment_utils.py

    if "prep_pipeline" in params:
        print("✅ Saving pipeline.json")
        with open(os.path.join(save_dir, "pipeline.json"), "w") as f:
            json.dump(params["prep_pipeline"], f, indent=4)
    else:
        print("❌ params['prep_pipeline'] not found, pipeline.json not saved")
...
def grid_search(experiment_obj, param_grid, verbose=True):
    ...
    for i, params in enumerate(get_param_candidates(param_grid)):
        ...
        if "no_crash" in params and params["no_crash"]:
            try:
                result, model, logger, prep_pipeline = experiment_obj.run(params, verbose=verbose)
            except:
                print("Error!!!!!!")
                continue
        else:
        ...
        #     if hasattr(prep_pipeline, "best_config"):
        #         best_params["prep_pipeline"] = prep_pipeline.best_config()
        if result["best_val_loss"] < best_val_loss:
            best_val_loss = result["best_val_loss"]
            best_result = result
            best_model = model
            ...
            if hasattr(prep_pipeline, "best_config"):
                best_params["prep_pipeline"] = prep_pipeline.best_config()
                print("🧪 Extracted pipeline config:", best_params["prep_pipeline"])
