...
class NumMVImputer(object):
    ...
    def __init__(self, method='mean'):
        ...
        if self.method == "mean":
            ...
            self.tf = IterativeImputer(random_state=0, skip_complete=False, max_iter=1000)
        elif self.method == "mode":
            self.tf = SimpleImputer(strategy='most_frequent')
        elif self.method == "DT":
...
class RowDeletionImputer:
    def __init__(self):
        self.method = "row_deletion"
        self.input_type = "numerical"
    ...
    def transform(self, X):
        ...
        if isinstance(X, np.ndarray):
            X = pd.DataFrame(X)
        X = X.dropna()
        return X.values.astype(np.float32)  # 👈 Force to float32 type!
