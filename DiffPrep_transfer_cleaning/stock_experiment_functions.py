...
def create_stock_features(df, target="Close", num_days_pred=30):
    ...
    # df.drop(columns=['Open', 'High', 'Low', 'Volume', "Adj Close", "Date"], inplace=True)
    return df
...
#     # partition_2.loc[:, 'Date'] = pd.to_datetime(partition_2['Date'], format='%Y-%m-%d')
...
def calculate_rank_difference_dataframe(partition_1, partition_2, distance):
    ...
    # Calculate rank within each model separately
    df_temp_1['Rank'] = df_temp_1.groupby(['Model'])['MSE'].rank(ascending=True)
    ...
    # Calculate rank within each model separately
    df_temp_2['Rank'] = df_temp_2.groupby(['Model'])['MSE'].rank(ascending=True)

    # print(df_temp_2)

    rank_diff = df_temp_1['Rank'].sub(df_temp_2['Rank'])
    ...
    # Add column to df_diff that is the air_quality experiment
    # df_temp_1['Experiment'] = experiment
...
def randomly_drop_data(df, target_columns, drop_ratio=0.2):
    if df.empty:
